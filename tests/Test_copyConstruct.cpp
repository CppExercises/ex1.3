...
int main() {
  ...
  const List list(numberOfElements, value);
  const auto originalData = list.data();
  const auto originalSize = list.size();
  
  List newList(list); // construct from old list

  // check validity of new list
  TEST_ASSERT(newList.data() != originalData, "Data was moved from old list instead of copied");
  ...
  TEST_ASSERT(list.data() != nullptr, "Old list has invalid data reference after copy");
